...
import { Search, Filter, User as UserIcon, Shield, Crown, ChevronRight } from 'lucide-react';
...
const PlayerGallery = () => {
    ...
    useEffect(() => {
        let result = players;
        if (search) {
            result = result.filter(p => p.name.toLowerCase().includes(search.toLowerCase()) || p.dept.toLowerCase().includes(search.toLowerCase()));
        }
        if (filter !== 'All') {
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                <header className="mb-12">
                    <div className="flex justify-between items-center mb-6">
                        <Link to="/" className="text-sm font-bold text-premium-gold hover:underline flex items-center gap-2 uppercase tracking-widest">
                            ← Back to Arena
                        </Link>
                        <div className="text-xs font-mono opacity-50 uppercase tracking-[0.3em]">Official Player Registry</div>
                    </div>
                    ...
                    <p className="text-gray-400 max-w-2xl border-l-2 border-premium-gold pl-4 text-sm md:text-base">
                        Discover the stars of the upcoming season. Browse through all registered athletes awaiting their moment in the spotlight.
                    </p>
                </header>
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0 custom-scrollbar">
                        {roles.map(role => (
                            <button
                                ...
                                className={`px-6 py-4 rounded-xl font-bold text-xs uppercase tracking-widest whitespace-nowrap transition-all border ${filter === role
                            ...
                            {filteredPlayers.map((player) => (
                                ...
                                >
                                    ...
                                    <div className="h-48 bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center relative overflow-hidden">
                                        <UserIcon size={80} className="text-gray-700 relative z-10 group-hover:scale-110 transition-transform duration-500" />
                                        <div className="absolute inset-0 bg-premium-gold/5 group-hover:bg-premium-gold/10 transition-colors"></div>

                                        {/* Status Ribbon */}
                                        <div className={`absolute bottom-0 left-0 w-full py-1.5 text-center text-[10px] font-bold uppercase tracking-[0.2em] z-20 ${player.status === 'sold' ? 'bg-green-600 text-white' :
                                                player.status === 'unsold' ? 'bg-red-600 text-white' :
                                                    player.status === 'approved' ? 'bg-premium-gold text-black' : 'bg-gray-700 text-gray-300'
                                            }`}>
                                            {player.status === 'pending' ? 'Verification Pending' : player.status}
                                        </div>
                                    </div>

                                    <div className="p-6">
                                        <h3 className="text-xl font-black italic mb-1 group-hover:text-premium-gold transition-colors">{player.name}</h3>
                                        ...
                                        <div className="grid grid-cols-2 gap-3 mb-6">
                                            <div className="bg-white/5 p-2 rounded-lg border border-white/5">
                                                <p className="text-[8px] text-gray-500 uppercase font-black mb-1">Base Price</p>
                                                <p className="text-sm font-mono font-bold">₹{player.basePrice}L</p>
                                            </div>
                                            {player.status === 'sold' ? (
                                                <div className="bg-green-500/10 p-2 rounded-lg border border-green-500/20">
                                                    <p className="text-[8px] text-green-500 uppercase font-black mb-1">Sold For</p>
                                                    <p className="text-sm font-mono font-bold text-green-400">₹{player.soldPrice}L</p>
                                                </div>
                                            ) : (
                                                <div className="bg-blue-500/5 p-2 rounded-lg border border-blue-500/10">
                                                    <p className="text-[8px] text-blue-400 uppercase font-black mb-1">Matches</p>
                                                    <p className="text-sm font-mono font-bold">{player.stats?.matches || 0}</p>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        {player.team && (
                                            <div className="flex items-center gap-2 p-2 bg-premium-gold/5 rounded border border-premium-gold/20">
                                                ...
                                                <span className="text-[10px] font-bold text-premium-gold uppercase">Squad: {player.team.name}</span>
                                            </div>
                                        )}

                                        {!player.team && player.status === 'approved' && (
                                            <p className="text-[10px] text-center italic text-gray-600">Awaiting bidding in live auction</p>
                                        )}
                                    </div>
                                </motion.div>
                            ))}
                ...
                {filteredPlayers.length === 0 && !loading && (
                    <div className="text-center py-24 glass-panel border-dashed border-2">
                        <p className="text-gray-500 mb-4 opacity-70">No players found matching your criteria.</p>
                        <button
