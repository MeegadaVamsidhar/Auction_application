  ...
  const fetchTeamData = async () => {
    try {
      ...
      if (user.team) {
        try {
          const res = await axios.get(
            `${API_URL}/api/admin/teams/${user.team}`,
          );
          teamInfo = res.data;
        } catch (e) {
      ...
      if (!teamInfo) {
        try {
          const res = await axios.get(
            `${API_URL}/api/admin/teams/captain/${user.id || user._id}`,
          );
          teamInfo = res.data;
      ...
      if (!teamInfo) {
        setError(
          "No team linked to your account. Please register your team first.",
        );
        setLoading(false);
        return;
      }
      ...
      if (teamInfo.status === "pending") {
        setError(
          "Your Team Registration is pending Admin approval. Please check back later.",
        );
        setLoading(false);
        return;
      ...
      if (err.response?.status === 404) {
        setError(
          "Team not found for this account. Your session might be outdated.",
        );
      } else {
  ...
  useEffect(() => {
    ...
    socketRef.current.on("auctionUpdate", (data) => {
      if (data.isActive) {
        ...
        const currentVal =
          data.highestBid === 0
            ? data.currentPlayer?.basePrice || 0
            : data.highestBid;
        const calculatedBid =
          data.highestBid === 0
            ? currentVal
            : currentVal + (data.bidIncreaseAmount || 5);
  ...
  if (loading)
    return (
      <div className="min-h-screen bg-premium-dark flex items-center justify-center">
        <div className="text-xl font-black italic text-premium-gold animate-pulse">
          LOADING SQUAD...
        </div>
      </div>
    );
          ...
          {error.includes("No team linked") ? (
            <div className="space-y-3">
              <p className="text-xs text-gray-500 mb-4">
                It looks like you don't have a team registered yet. Please
                register your team to continue.
              </p>
              <button
                onClick={() => {
            ...
            <div className="space-y-3">
              <div className="bg-yellow-900/20 border border-yellow-600/30 rounded p-4 mb-4">
                <p className="text-yellow-400 text-xs">
                  ⏳ Your team registration is awaiting admin approval. You'll
    ...
    <div className="min-h-screen bg-premium-dark p-4 md:p-6 text-white grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-1 space-y-6 order-2 lg:order-1">
        ...
        <div className="premium-card p-6 border-l-4 border-premium-gold">
          <h1 className="text-2xl font-black italic uppercase text-white mb-1">
            {team.name}
          </h1>
          <p className="text-xs opacity-50 uppercase tracking-widest mb-4">
            Captain Dashboard
          </p>
        ...
        <div className="premium-card p-4 h-[600px] overflow-y-auto">
          <h3 className="text-lg font-bold mb-4 border-b border-gray-800 pb-2 bg-black/50 p-2 sticky top-0">
            YOUR SQUAD
          </h3>
          <div className="space-y-3">
            {team.players &&
              team.players.map((player) => (
                <div
                  key={player._id}
                  className="bg-black/30 p-3 rounded border border-gray-800 flex justify-between items-center group hover:border-premium-gold/30 transition-all"
                >
                  <div>
                  ...
                  <p className="font-mono text-premium-gold font-bold">
      ...
      <div className="lg:col-span-2 order-1 lg:order-2">
        {liveAuction ? (
          <div className="premium-card h-full flex flex-col relative overflow-hidden border-2 border-premium-gold/20">
            <div className="absolute top-0 right-0 bg-red-600 text-white text-xs font-bold px-3 py-1 animate-pulse">
              LIVE
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-6 p-6 border-b border-gray-800">
              ...
              <div>
                ...
                <p className="text-lg opacity-60 mb-1">
                  ...
                  {liveAuction.currentPlayer?.dept} | Year:{" "}
                  {liveAuction.currentPlayer?.year}
                </p>
                {liveAuction.currentPlayer?.previousTeams && (
                  <p className="text-premium-gold font-bold text-sm mb-1 uppercase tracking-tight">
                    Prev Teams: {liveAuction.currentPlayer.previousTeams}
                  </p>
                )}
                <p className="text-premium-gold font-black italic mb-4">
                  BASE PRICE:{" "}
                  {formatPrice(liveAuction.currentPlayer?.basePrice)}
                </p>
                <div className="flex gap-4 text-sm font-mono text-gray-400">
                  <span className="bg-black/40 px-3 py-1 rounded">
                    Matches: {liveAuction.currentPlayer?.stats?.matches}
                  </span>
                  <span className="bg-black/40 px-3 py-1 rounded">
                    Runs: {liveAuction.currentPlayer?.stats?.runs}
                  </span>
                  <span className="bg-black/40 px-3 py-1 rounded">
                    Wickets: {liveAuction.currentPlayer?.stats?.wickets}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-0 flex-1">
              ...
              <div className="p-6 bg-black/20 flex flex-col justify-center items-center border-r border-gray-800">
                <p className="text-sm opacity-50 uppercase mb-2">
                  Current Price
                </p>
                <div className="text-5xl font-black text-white mb-2 font-mono">
                  {formatPrice(
                    liveAuction.highestBid === 0
                      ? liveAuction.currentPlayer?.basePrice
                      : liveAuction.highestBid,
                  )}
                </div>
                ...
                <button
                  ...
                  className={`w-full max-w-xs py-4 text-xl font-black italic rounded-lg transition-all transform active:scale-95 ${
                ...
                <p className="text-xs text-gray-500 mt-4">
                  Timer: {liveAuction.timer}s
                </p>
              </div>

              {/* Bid Log */}
              <div className="p-6 bg-black/40 overflow-y-auto max-h-[400px]">
                <h4 className="text-xs font-bold opacity-50 uppercase mb-4 sticky top-0 bg-[#121212] py-2">
                  Real-time Activity
                </h4>
                <div className="space-y-3 font-mono text-sm">
                  {liveAuction.bidHistory &&
                    liveAuction.bidHistory.map((log, idx) => (
                      <div
                        key={idx}
                  ...
                  {(!liveAuction.bidHistory ||
                    liveAuction.bidHistory.length === 0) && (
                    <p className="text-xs opacity-30 italic text-center mt-10">
                      Bidding has started. Waiting for first bid...
                    </p>
                  )}
                </div>
              </div>
          ...
          <div className="premium-card h-full flex items-center justify-center flex-col opacity-50 p-12 text-center border-dashed border-2 border-gray-700">
            ...
            <p className="max-w-md">
              The admin has not started the bidding for the next player yet.
              Review your strategy while you wait.
            </p>
          </div>
        )}
      </div>
