    ...
    <div className="min-h-screen bg-premium-dark p-4 md:p-8">
      <header className="flex flex-col md:flex-row justify-between items-center mb-8 md:mb-12 border-b border-premium-border pb-6 gap-4">
        <h1 className="text-2xl md:text-4xl font-extrabold italic text-premium-gold tracking-tighter text-center md:text-left">
          LIVE AUCTION{" "}
          <span className="text-white text-lg md:text-xl ml-2 font-normal not-italic block md:inline">
            ARENA 2026
          </span>
        </h1>
        <div className="flex items-center gap-6">
          {user ? (
            <div className="flex items-center gap-4">
              <span className="text-sm font-bold text-gray-400 uppercase hidden md:inline">
            ...
            <Link
              to="/login"
              className="text-sm font-bold text-premium-gold hover:underline"
            >
              LOGIN
            </Link>
          )}
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-12 gap-6 lg:gap-8">
          ...
          <AnimatePresence mode="wait">
            {auctionData?.currentPlayer ? (
              ...
              >
                ...
                <div className="flex flex-col md:flex-row gap-6 md:gap-8 p-2 md:p-4">
                  <div className="w-full md:w-1/2 aspect-square bg-black/50 rounded-lg flex items-center justify-center border border-premium-border">
                    <UserIcon
                      size={120}
                      className="text-gray-700 w-24 h-24 md:w-32 md:h-32"
                    />
                  </div>
                  <div className="w-full md:w-1/2 flex flex-col justify-center text-center md:text-left">
                    ...
                    <p className="text-lg md:text-xl opacity-60 mb-6">
                      {auctionData.currentPlayer.dept} |{" "}
                      {auctionData.currentPlayer.year} Year
                    </p>
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-6 bg-black/30 p-4 rounded-lg border border-premium-border/50 text-center">
                      <div>
                        ...
                        <p className="font-mono font-bold">
                          {auctionData.currentPlayer.stats?.matches || "0"}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs opacity-50 uppercase">Runs</p>
                        <p className="font-mono font-bold text-premium-gold">
                          {auctionData.currentPlayer.stats?.runs || "0"}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs opacity-50 uppercase">Wickets</p>
                        <p className="font-mono font-bold">
                          {auctionData.currentPlayer.stats?.wickets || "0"}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-8">
                      <div>
                        ...
                        <p className="text-lg md:text-xl font-mono">
                          {formatPrice(auctionData.currentPlayer.basePrice)}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs opacity-50 uppercase">
              ...
              <div className="premium-card h-[300px] md:h-[400px] flex items-center justify-center">
                <p className="text-xl md:text-2xl opacity-30 italic text-center px-4">
        ...
        <div className="order-3 lg:order-1 lg:col-span-3 space-y-4">
          <h2 className="text-lg md:text-xl font-bold mb-4 opacity-70 border-b border-gray-700 pb-2 lg:border-none lg:pb-0">
            TEAM STANDINGS
          </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4">
            {teams
              ...
              .map((team) => (
                <div
                  key={team._id}
                  className="premium-card p-4 flex justify-between items-center bg-black/30"
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs opacity-50">
                      SQUAD: {team.players?.length || 0}/{team.maxPlayers || 15}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-premium-gold font-mono text-sm md:text-base">
