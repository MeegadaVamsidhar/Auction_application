...
router.post('/register', async (req, res) => {
    try {
        ...
        const { name, mobile, password, dept, year, role, battingStyle, bowlingStyle, contact, stats, previousTeams } = req.body;

        // Validation
        if (!name || !mobile || !dept || !year || !role) {
            return res.status(400).json({ error: 'Please fill all required fields: Name, Mobile, Dept, Year, Role' });
        }

        // Check if player already exists with same mobile
        const existingPlayer = await Player.findOne({ mobile });
        if (existingPlayer) {
            return res.status(400).json({ error: 'A player with this mobile number is already registered' });
        }
        ...
        if (!validRoles.includes(normalizedRole)) {
            return res.status(400).json({ error: `Invalid role selected. Must be one of: ${validRoles.join(', ')}` });
        }
        ...
        res.status(201).json({ message: 'Registration Successful! Waiting for Admin Approval.', player: { id: newPlayer._id, name: newPlayer.name } });
    } catch (err) {
